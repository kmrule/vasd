...
function testYTB() { 
  ...
  $task.fetch(option).then(response => {
    ...
    if (response.statusCode !== 200) {
      ...
      let re = new RegExp('<p><a class="YfaGG" href="terms\\?hl=zh-CN#footnote-country-version" data-name="country-version" jsaction="click:IPbaae\\(preventDefault=true\\)">国家/地区版本：</a>\\s*(.*?)</p>', 'g')
      let match = re.exec(data)
      if (match !== null && match.length === 2 && match[1].indexOf("中国") !== -1) {
        result["YouTube"] = "<b>YouTube Premium: </b>送中 😡"
      } else {
        result["YouTube"] = "<b>YouTube Premium: </b>未送中 ✅"
      }
    }
  }, reason => {
