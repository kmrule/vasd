...
function testGoogle(pname) {
    return new Promise((resolve, reject) => {
        ...
        $task.fetch(myRequest).then((response) => {
            ...
            const country = dom.window.document.querySelector("html > body > c-wiz > div > div:nth-child(2) > div:nth-child(2) > div:nth-child(3) > c-wiz > div:nth-child(1) > div > p:nth-child(2)").textContent;

            if (country.includes("中国")) {
                console.log("送中了，" + country);
                resolve();
            } else {
                console.log("没有送中，" + country);
                reject();
            }
        }).catch((error) => {
            console.error(error);
